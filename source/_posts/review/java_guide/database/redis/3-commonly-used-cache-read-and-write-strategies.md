...
Read/Write Through Pattern 中服务端**把 cache 视为主要数据**存储，从中读取数据并将数据写入其中。c**ache 服务**负责将此**数据读取**和**写入 db**，从而减轻了应用程序的职责。
...
这种策略在我们平时开发过程中也非常非常少见，但是不代表它的应用场景少，比如消息队列中消息的**异步写入磁盘**、MySQL 的 **Innodb Buffer Pool 机**制都用到了这种策略。
