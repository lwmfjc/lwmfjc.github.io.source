...
- I/O(**Input/Output**)，即**输入/输出**
  ...
  ![lyx-20241126133709271](images/mypost/lyx-20241126133709271.png)
  其中，输入设备：键盘；输出设备：显示器 
  网卡、硬盘既属于输入设备也属于输出设备
...
- 应用程序发起read调用后，会一直阻塞，**直到内核把数据拷贝到用户空间**
  ![lyx-20241126133709817](images/mypost/lyx-20241126133709817.png)
...
- NIO是I/O多路复用模型，属于**同步非阻塞IO**模型
  - **一般的同步非阻塞 IO 模型**中，应用程序会**一直发起 read** 调用。  
    ...
    ![图源：《深入拆解Tomcat & Jetty》](images/mypost/lyx-20241126133710258.image)  
  ...
  - Selector，即多路复用器，一个线程管理多个客户端连接
    ![lyx-20241126133710714](images/mypost/lyx-20241126133710714.png)

### AIO(Asynchronous I/O )

- 异步 IO 是基于**事件和回调**机制实现的，也就是**应用操作之后会直接返回**，不会堵塞在那里，当后台处理完成，操作系统会通知相应的线程进行后续的操作
  如图  
  ![lyx-20241126133711142](images/mypost/lyx-20241126133711142.png)

## 三者区别

![lyx-20241126133711604](images/mypost/lyx-20241126133711604.png)
