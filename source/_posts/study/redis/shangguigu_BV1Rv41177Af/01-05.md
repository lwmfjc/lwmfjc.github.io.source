...
* 大数据时代（行式数据库、列式数据库）
  * 行式数据库  
  查询某一块数据的时候效率高  
  ![](images/mypost/1640929802056.png)  
  * 列式数据库  
  查询某一列统计信息快  
  ![](images/mypost/1640929838300.png)    
  * 其他  
...
* Redis功能
  ...
  * 其他部分功能   
  ![](images/mypost/1640929869759.png)  
* Redis安装  
  ...
  * 进入/usr/local/bin目录，查看目录  
  ![](images/mypost/1640939791651.png)
* Redis启动
  * 前台启动
    ``` 
    redis-server 
...
* Redis6379的由来
  * 人名Merz 在九宫格对应的数字就是6379  
  ![](images/mypost/1641283251602.png)
* Redis默认有15个库，默认数据都在数据库0中，所有库的密码都是相同的
* Redis是单线程+多路复用技术  
  * Redis是串行操作  
    ![](images/mypost/1641283379975.png)
  * 火车站的例子  
  当1，2，3没有票的时候，不用一直等待买票，可以继续做自己的事情，黄牛买到票就会通知123进行取票  
  ![](images/mypost/1641283510456.png)
* Memcached和Redis区别  
