...
1. 类加载检查
   虚拟机遇到一条**new指令**时，首先将去检查这个指令的**参数**是否能在**常量池中**定位到这个**类的符号引用**，并且检查这个符号引用代表的类**是否已被加载过**、**解析和初始化**过。**如果没有**，那必须先执行相应的**类加载**过程
...
2. 分配内存
   ...
   **内存分配并发问题**：
   ...
   创建对象时的重要问题---线程安全，因为在实际开发过程中，**创建对象是很频繁的事**，作为虚拟机来说，必须要保证线城市安全的，虚拟机采用**两种方式保证线程安全**：  
...
建立对象就是为了**使用对象**，我们的 Java 程序通过**栈上的 reference 数据**来**操作堆上的具体对象**。对象的访问方式由虚拟机实现而定，目前**主流的访问方式**有：**使用句柄**、**直接指针**。
