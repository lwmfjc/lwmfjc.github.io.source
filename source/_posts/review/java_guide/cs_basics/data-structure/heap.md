...
- 下面给出的图是否是堆（通过定义）
  ...
  1，2是。
  3不是。
  ![lyx-20241126133423794](attachments/img/lyx-20241126133423794.png)
...
- 如图，图1是最大堆，图2是最小堆
  ![lyx-20241126133424328](attachments/img/lyx-20241126133424328.png)
...
- 为了**方便存储**和**索引**，**（二叉）堆**可以用**完全二叉树**的形式进行存储。存储的方式如下图所示
  ![lyx-20241126133424772](attachments/img/lyx-20241126133424772.png)
...
1. 将要插入的元素放到**最后**
   ![lyx-20241126133425184](attachments/img/lyx-20241126133425184.png)
2. 从底向上，如果**父节点**比**该元素小**，则该节点和父节点交换（其实就是一棵树有3个（最多）节点，与树上最大的节点比较）
   直到无法交换（已经与根节点比较过）
   ![lyx-20241126133425732](attachments/img/lyx-20241126133425732.png)
   ![lyx-20241126133426224](attachments/img/lyx-20241126133426224.png)
...
- **自底向上堆化**
  ...
  1. 首先删除堆顶元素，使得数组中下标为1的位置空出
     ![lyx-20241126133426634](attachments/img/lyx-20241126133426634.png)

     > 那么他的位置由谁来接替呢，当然是他的直接下属了，谁能力强就让谁上

  2. 比较**根节点（当前节点）**的**左子节点**和**右子节点**，也就是下标为 2 ，3 的数组元素，将较大的元素填充到**根节点（下标为1）（当前遍历节点）**的位置
     ![lyx-20241126133427112](attachments/img/lyx-20241126133427112.png)

     > 此时又空出一个位置了，老规矩，谁有能力谁上

  3. 一直循环比较**空出位置**的**左右子节点**，并将较大者移至空位，直到堆的最底部
     ![lyx-20241126133427538](attachments/img/lyx-20241126133427538.png)
...
- 自顶向下堆化
  ...
  1. 第一件事情，就是把石头抬起来，从海面扔下去。这个石头就是**堆的最后一个元素**，我们**将最后一个元素移动到堆顶**。
     ![lyx-20241126133428070](attachments/img/lyx-20241126133428070.png)
  2. 将这个石头沉入海底，不停的与**左右子节点**的值进行比较，和**较大的子节点**交换位置，直到**无法交换位置**
     ![lyx-20241126133428507](attachments/img/lyx-20241126133428507.png)
  3. 结果
     ![lyx-20241126133428992](attachments/img/lyx-20241126133428992.png)
...
- 也就是对**所有非叶子结点**进行自顶向下
  ...
  ![lyx-20241126133429421](attachments/img/lyx-20241126133429421.png)
  这个构建堆操作的时间复杂度为O(n)
  ![lyx-20241126133429835](attachments/img/lyx-20241126133429835.png)

- 首先要了解哪些是非叶节点，**最后一个结点的父节点及它（这个父节点）之前的元素**，都是非叶节点。也就是说，如果**节点个数为n**，那么我们需要对**n/2到1的节点进行自顶向下（沉底）堆化**

- 如图
  ![lyx-20241126133430306](attachments/img/lyx-20241126133430306.png)

  1. 首先将初始的无序数组抽象为一棵树，图中的节点个数为6，所以4，5，6是叶子节点，1，2，3节点为非叶节点  
  2. 对1，2，3节点进行**自顶向下（沉底）**堆化，注意，顺序是从后往前堆化，从3号开始，一直到1号节点。
     - 3号节点堆化结果  
       ![lyx-20241126133430752](attachments/img/lyx-20241126133430752.png)
     - 2号节点堆化结果
       ![lyx-20241126133431255](attachments/img/lyx-20241126133431255.png)
     - 1号节点堆化结果
       ![lyx-20241126133431683](attachments/img/lyx-20241126133431683.png)
...
- 步骤
  1. 取出第一个元素并堆化
     ![lyx-20241126133432109](attachments/img/lyx-20241126133432109.png)
  2. 取出第2个元素并堆化
     ![lyx-20241126133432600](attachments/img/lyx-20241126133432600.png)
  3. 取出第3个元素并堆化
     ![](attachments/img/lyx-20241126133433016.png)
  4. 取出第4个元素并堆化
     ![lyx-20241126133433455](attachments/img/lyx-20241126133433455.png)
  5. 取出第5个元素并堆化
     ![lyx-20241126133433933](attachments/img/lyx-20241126133433933.png)
  6. 取出第6个元素并堆化
     ![lyx-20241126133434407](attachments/img/lyx-20241126133434407.png)
  7. 排序完成