...
- mysql执行  
  ![lyx-20241126133500628](attachments/img/lyx-20241126133500628.png)

- 总结  
  ![lyx-20241126133500760](attachments/img/lyx-20241126133500760.png)
...
![lyx-20241126133501323](attachments/img/lyx-20241126133501323.png)
...
> 再具体点：防止在**发生故障的时间点**，尚有**脏页未写入磁盘**，在重启mysql服务的时候，根据redo log进行重做，从而达到事务的持久性这一特性
  ...
  ![lyx-20241126133501771](attachments/img/lyx-20241126133501771.png)
...
2. 把“**在某个数据页上做了什么修改**”记录到**重做日志缓存**（`redo log buffer`）里，接着**刷盘到 `redo log` 文件**里  
   ...
   ![lyx-20241126133502204](attachments/img/lyx-20241126133502204.png)
...
![lyx-20241126133502643](attachments/img/lyx-20241126133502643.png)

![lyx-20241126133503069](attachments/img/lyx-20241126133503069.png)
...
### innodb_flush_log_at_trx_commit=0（不对是否刷盘做出处理）  
  ...
  ![lyx-20241126133503497](attachments/img/lyx-20241126133503497.png)

### innodb_flush_log_at_trx_commit=1  

  > 为`1`时， **只要事务提交成功**，**`redo log`记录就一定在硬盘里**，不会有任何数据丢失。如果事务执行期间`MySQL`挂了或宕机，这部分日志丢了，但是事务并没有提交，所以日志丢了也不会有损失。

  ![lyx-20241126133503956](attachments/img/lyx-20241126133503956.png)
...
### innodb_flush_log_at_trx_commit=2
  ...
  ![lyx-20241126133504419](attachments/img/lyx-20241126133504419.png)
...
- 它采用的是**环形数组形式**，从头开始写，写到末尾又回到头循环写，如下图所示  
  ![lyx-20241126133504871](attachments/img/lyx-20241126133504871.png)
...
![lyx-20241126133505300](attachments/img/lyx-20241126133505300.png)
...
![lyx-20241126133505724](attachments/img/lyx-20241126133505724.png)
...
- MySQL的数据库的**数据备份**、**主备**、**主主**、**主从**都离不开binlog，需要依靠binlog来**同步数据**，**保证数据一致性**。
  ![lyx-20241126133506137](attachments/img/lyx-20241126133506137.png)
...
1. 指定**`statement`**，记录的内容是**`SQL`语句原文**，比如执行一条`update T set update_time=now() where id=1`，记录的内容如下
   ![lyx-20241126133506596](attachments/img/lyx-20241126133506596.png)
...
2. 为了解决上面问题，需要指定**row**，记录的不是简单的SQL语句，还包括**操作的具体数据**，记录内容如下  
   ...
   ![lyx-20241126133507059](attachments/img/lyx-20241126133507059.png)
...
- 我们可以通过`binlog_cache_size`参数控制**单个线程 binlog cache 大小**，如果存储内容超过了这个参数，就要暂存到磁盘（`Swap`）：  
  binlog日志刷盘流程如下  
  ![lyx-20241126133507479](attachments/img/lyx-20241126133507479.png)
...
- write和fsync的时机，由**sync_binlog**控制，默认为0
  ...
  1. 为**0**时，表示每次提交的事务都只**write**，由系统自行判断什么时候执行fsync
     ![lyx-20241126133507904](attachments/img/lyx-20241126133507904.png)

     > 虽然性能会提升，但是如果机器宕机，**page cache**里面的binlog会**丢失**

  2. 设置为**1**，表示**每次提交事务**都会fsync ，就如同**redo log日志刷盘流程** 一样

  3. 折中，可以设置为**N(N>1)**，表示每次提交事物都write，但累积**N**个事务之后才**fsync**![lyx-20241126133508315](attachments/img/lyx-20241126133508315.png)
...
![lyx-20241126133508740](attachments/img/lyx-20241126133508740.png)
...
- 假设执行过程中**写完redo log**日志后，**binlog日志写期间发生了异常**，会出现什么情况
  ![lyx-20241126133509206](attachments/img/lyx-20241126133509206.png)
...
- 为了解决**两份日志之间的逻辑一致**问题，InnoDB存储引擎使用**两阶段提交**方案
  ...
  > ![lyx-20241126133510596](attachments/img/lyx-20241126133510596.png)
  >
  > 还有个问题，**`redo log`设置`commit`阶段发生异常**，那会不会回滚事务呢？    
  >  ![lyx-20241126133511041](attachments/img/lyx-20241126133511041.png)
  >
...
- 三大日志大概的流程
  ![lyx-20241126133511576](attachments/img/lyx-20241126133511576.png)
