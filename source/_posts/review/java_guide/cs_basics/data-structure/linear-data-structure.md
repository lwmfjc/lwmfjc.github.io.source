...
![lyx-20241126133434855](images/mypost/lyx-20241126133434855.png)
...
- 如下图
  ![lyx-20241126133435394](images/mypost/lyx-20241126133435394.png)

### 循环链表

- 循环链表是一种**特殊的单链表**，和单链表不同的是**循环链表的尾结点**不是指向null，而是**指向链表的头结点**
- 如图
  ![lyx-20241126133435817](images/mypost/lyx-20241126133435817.png)

### 双向链表

- 双向链表包含**两个指针**，一个**prev**指向**前一个节点**，另一个**next**指向
- 如图
  ![lyx-20241126133436363](images/mypost/lyx-20241126133436363.png)

### 双向循环链表

双向循环链表的**最后一个节点的next**指向head，而head的**prev**指向最后一个节点，构成一个环

![lyx-20241126133436788](images/mypost/lyx-20241126133436788.png)
...
如图：  
![lyx-20241126133437269](images/mypost/lyx-20241126133437269.png)
...
![lyx-20241126133437786](images/mypost/lyx-20241126133437786.png)
...
- 队列的操作方式和堆栈类似，唯一的区别在于**队列**只允许**新数据在后端**进行添加（不允许在后端删除）
  ...
  > 插入删除：O（1）//后端插入前端删除元素
  >
  > ![队列](images/mypost/lyx-20241126133438282.png)
...
![lyx-20241126133438734](images/mypost/lyx-20241126133438734.png)
...
![lyx-20241126133439188](images/mypost/lyx-20241126133439188.png)
（超出的时候，将rear指向0下标）。之后再添加时，向后移动即可

- 顺序队列中，我们说 `front==rear` 的时候队列为空，循环队列中则不一样，也可能为满，如上图所示。解决办法有两种：
  1. 可以设置一个标志变量 `flag`,当 `front==rear` 并且 `flag=0` 的时候队列为空，当`front==rear` 并且 `flag=1` 的时候队列为满。
  2. 队列为空的时候就是 `front==rear` ，**队列满的时候，我们保证数组还有一个空闲的位置**，rear 就指向这个空闲位置，如下图所示，那么现在判断队列是否为满的条件就是： `(rear+1) % QueueSize= front` 。
     ![lyx-20241126133439611](images/mypost/lyx-20241126133439611.png)
     其实也就是换一个定义罢了
