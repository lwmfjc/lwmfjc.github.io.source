...
> 6. 存储引擎**将回滚需要的数据记录到 Undo Log**，并将这个**更新操作记录到 Redo Log**，此时 Redo Log 处于 prepare 状态。并将这行新数据更新到内存（innodb_buffer_cache）中。同时，然后告知执行器执行完成了，随时可以提交事务。
> 7. **手动事务 commit**：执行器生成这个操作的 Binary Log，并把 Binary Log 写入磁盘。（**binlog：提交事务时写入**）
