...
- 多数情况下，对象在**新生代中Eden区**分配。当Eden区没有足够空间进行分配时，会触发一次MinorGC
  ...
  ![img](attachments/img/lyx-20241126133720742.jpg)
  ...
  ![img](attachments/img/lyx-20241126133721387.jpg)
...
如图：（太长跳过了，直接看下面的总结）  
![img](attachments/img/lyx-20241126133721837.png)
...
- 该算法的基本思想就是通过一系列称为**“GC Roots"**的对象作为起点，从这些节点开始**向下搜索**，节点所走过的**路径** 称为**引用链**，当一个对象到GC Roots**没有任何引用链**相连的话，证明该对象不可用，需要**被回收**
  ...
  ![可达性分析算法](attachments/img/lyx-20241126133722264.png)
...
![lyx-20241126133722746](attachments/img/lyx-20241126133722746.png)
...
![复制算法](attachments/img/lyx-20241126133723220.png)

## 标记-整理算法

根据老年代特点提出的一种**标记算法**，**标记过程**仍然与**“标记-清除”**算法一样，但后续不是直接对可回收对象回收，而是让**所有存活对象向一端移动**，然后直接**清理掉端边界以外的内存**

![标记-整理算法 ](attachments/img/lyx-20241126133723760.png)
...
![img](attachments/img/lyx-20241126133724232.png)
...
- 这是一个**单线程收集器**，它的**单线程**意义不仅意味着它只会使用**一条垃圾收集线程**去完成垃圾收集工作，更重要的是它在**进行垃圾收集工作时**必须暂停其他所有的工作线程**（”Stop The World“）**，直到它**收集结束**。
  ...
  - 新生代采用**标记-复制**算法，老年代采用**标记-整理**算法
    ![ Serial 收集器 ](attachments/img/lyx-20241126133724253.png)
...
- **新生代**采用**标记-复制**算法，**老年代**采用**标记-整理**算法
  ...
  ![ParNew 收集器 ](attachments/img/lyx-20241126133724714.png)
...
- 它也是**标记-复制**算法的多线程收集器，看上去几乎和**ParNew**一样，**区别**
  ...
  - 新生代采用**标记-复制**，老年代采用**标记-整理**算法
      ![lyx-20241126133725189](attachments/img/lyx-20241126133725189.png)
...
- Mark-Sweep，是一种“**标记-清除**”算法，运作过程相比前面几种垃圾收集器来说更加复杂，步骤：
  ...
  ![CMS 垃圾收集器 ](attachments/img/lyx-20241126133725632.png)
...
- 从名字可以看出这是一款优秀的收集器：**并发收集**、**低停顿**。但有三个明显缺点
  ...
  2. 无法处理浮动垃圾
     ...
     > ![img](attachments/img/lyx-20241126133726085.png)
     >
     > 作者：Yellowtail
