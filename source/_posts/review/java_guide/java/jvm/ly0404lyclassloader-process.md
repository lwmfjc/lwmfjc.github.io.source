...
![lyx-20241126133733684](images/mypost/lyx-20241126133733684.png)

# 类加载过程

- Class文件，需要**加载到虚拟机中**之后才能运行和使用，那么虚拟机是**如何加载**这些Class文件呢
- 系统加载Class类文件需要三步：**加载**->**连接**->**初始化**。连接过程又分为三步：**验证**->**准备**->**解析**  
  ![lyx-20241126133734207](images/mypost/lyx-20241126133734207.png)
...
![lyx-20241126133734818](images/mypost/lyx-20241126133734818.png)
...
- 基本数据类型的零值
  ![lyx-20241126133735234](images/mypost/lyx-20241126133735234.png)
...
- **符号引用**就是**一组符号**来**描述目标**，可以是任何**字面量**。**直接引用**就是**直接指向目标的指针**、**相对偏移量**或一个**间接定位到目标**的句柄
  ...
  >     >
  >     >    ![lyx-20241126133735715](images/mypost/lyx-20241126133735715.png)
  >     >
  >     > 2. 如下，可能会有好几个方法，所以方法表，其实是一个**类数组结构**，而每个方法信息（method_info）呢，![lyx-20241126133736282](images/mypost/lyx-20241126133736282.png)
  >     >
  >     > 3. 进一步，对于每个method_info结构体的定义  
  >     >    方法表的结构体由：**访问标志(\**access_flags\**)、名称索引(\**name_index\**)、描述索引(\**descriptor_index\**)、属性表(\**attribute_info\**)集合**组成。
  >     >
  >     >    > ![lyx-20241126133736720](images/mypost/lyx-20241126133736720.png)
  >     >
  >     > 4. 而对于属性表，（其中：属性表集合--用来记录方法的机器指令和抛出异常等信息）  
  >     >    ![img](images/mypost/lyx-20241126133737155.jpg)
  >     >    **Java之所以能够运行，就是从Code属性中，取出的机器码**  
  >     >
  >     >    ![lyx-20241126133737597](images/mypost/lyx-20241126133737597.png)
  >     >    ![img](images/mypost/lyx-20241126133738045.jpg)
...
- 解析阶段是虚拟机将**常量池内的符号引用**替换为**直接引用**的过程，也就是**得到类**或者**字段**、**方法**在内存中的**指针**或者**偏移量**。（因为此时那些class文件已经早就加载到**方法区**之中了，所以可以改成指向**方法区**的某个**内存地址**  
  ...
  ![lyx-20241126133738518](images/mypost/lyx-20241126133738518.png)  
  ![lyx-20241126133738931](images/mypost/lyx-20241126133738931.png)
