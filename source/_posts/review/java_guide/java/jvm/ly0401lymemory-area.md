...
![lyx-20241126133712055](attachments/img/lyx-20241126133712055.png)

JDK1.8之后：  
![Java 运行时数据区域（JDK1.8 之后）](attachments/img/lyx-20241126133712597.png)
...
- java程序流程  
  ![lyx-20241126133713026](attachments/img/lyx-20241126133713026.png)
...
![Java 虚拟机栈](attachments/img/lyx-20241126133713463.png)
...
- 局部变量表：存放**编译器可知**的各种**数据类型**(boolean、byte、char、short、int、float、long、double)、对象引用(reference 类型，不同于对象本身，可能是一个指向对象起始地址的引用指针，也可能是一个**指向一个代表对象的句柄或其他与此对象相关的位置**)
  ![局部变量表](attachments/img/lyx-20241126133713903.png)
...
- 动态链接  主要服务**一个方法需要调用其他方法**的场景。
  ...
  ![img](attachments/img/lyx-20241126133714347.png)
...
- 总结，程序运行中栈可能出现的两种错误
  ...
  > ![img](attachments/img/lyx-20241126133714807.png)
...
- JDK7及JDK7之前，堆内存被分为三部分
  ...
  ![hotspot-heap-structure](attachments/img/lyx-20241126133715264.png)
...
- 大部分情况，对象都会首先在 Eden 区域分配，在一次新生代垃圾回收后，如果对象还存活，则会进入 S0 或者 S1，并且对象的年龄还会加 1(**Eden 区->Survivor 区后对象的初始年龄变为 1**)，当它的年龄增加到一定程度（默认为 15 岁），就会被晋升到老年代中。
  ...
  > ![lyx-20241126133715694](attachments/img/lyx-20241126133715694.png)
  >
  > ---
  >
  > 代码如下：  
...
- **方法区和永久代以及元空间有什么关系呢？**
  ...
  ![HotSpot 虚拟机方法区的两种实现](attachments/img/lyx-20241126133716161.png)

- 为什么将永久代（PermGen）替换成元空间（MetaSpace）呢

  > 下图来自《深入理解Java虚拟机》第3版 
  >
  > ![img](attachments/img/lyx-20241126133716597.png)
...
- Class文件中除了有**类的版本**、**字段**、**方法**、**接口**等描述信息外，还有用于存放编译器期生成的**各种字面量（Literal）**和**符号引用（Symbolic Reference）**的**常量池表（注意，这里的常量池表，说的是刚刚编译后的那个class文件字节码代表的含义）**  
  ...
  > ![lyx-20241126133717005](attachments/img/lyx-20241126133717005.png)
...
- **常量池表**会在类加载后存放到**方法区**的**运行时常量池**中   
  ![lyx-20241126133717025](attachments/img/lyx-20241126133717025.png)
...
![lyx-20241126133717581](attachments/img/lyx-20241126133717581.png)

![lyx-20241126133718011](attachments/img/lyx-20241126133718011.png)
...
![lyx-20241126133718432](attachments/img/lyx-20241126133718432.png)

---

![lyx-20241126133718864](attachments/img/lyx-20241126133718864.png)

---

![lyx-20241126133719295](attachments/img/lyx-20241126133719295.png)
...
1. 句柄
   ...
   【也就是多了一层】![对象的访问定位-使用句柄](attachments/img/lyx-20241126133719855.png)

2. 直接指针
   如果使用**直接指针**访问，**reference 中存储的直接就是对象的地址**。
   ![对象的访问定位-直接指针](attachments/img/lyx-20241126133720278.png)
